...
public class Inventory implements java.io.Serializable
{
	...
	public void addItem(Item item, int count)
	{
		...
		for(int i = 0; i < inventoryItems.length && itemsToGo > 0; i++)
			...
			{
					if((j < craftingHeight && i < inventoryItems.length - tableSizeAvailable) || (craftingHeight != tableSizeAvailable && j == tableSizeAvailable))
						continue;
					itemsToGo = inventoryItems[i][playerRow].add(item, count);
			}
	}
	...
	public boolean updateInventory(int screenWidth, int screenHeight, int mouseX, int mouseY, boolean leftClick, boolean rightClick)
	{
		if(!visible)
			return false;
		...
		if(position != null)
		{
			if(holding.isEmpty())
			{
				if(rightClick && inventoryItems[position.x][position.y].count > 1)
				{
					...
					holding.count =  (int) Math.ceil((double)inventoryItems[position.x][position.y].count/2);
					inventoryItems[position.x][position.y].count = (int) Math.floor((double)inventoryItems[position.x][position.y].count/2);
				}
				else
				{
					holding.item = inventoryItems[position.x][position.y].item;
		...
		if(mouseX >= x && mouseX <= x + tileSize+10 && mouseY >= y && mouseY <= y + tileSize*2+10)
		{
	...
	private Int2 mouseToCoor(int x, int y, int seperation, int tileSize)
	{
		...
		if(clickPos.x < 0 || clickPos.y  < 0 || clickPos.x >= inventoryItems.length || clickPos.y >= inventoryItems[0].length || 
				((clickPos.y < craftingHeight && clickPos.x < inventoryItems.length - tableSizeAvailable) || (craftingHeight != tableSizeAvailable && clickPos.y == tableSizeAvailable)))
			return null;
		return clickPos;
	}
	...
	public void draw(Graphics2D g, int screenWidth, int screenHeight)
	{
		...
		g.fillRect(x + panelWidth - tableSizeAvailable*(tileSize + seperation) - seperation , y , tableSizeAvailable*(tileSize + seperation) + seperation, tableSizeAvailable*(tileSize + seperation) + seperation);
		...
		for(int i = 0; i < inventoryItems[0].length; i++)
		{
			...
			for(int j = 0; j < inventoryItems.length; j++)
			{
				if((i < craftingHeight && j < inventoryItems.length - tableSizeAvailable) || (craftingHeight != tableSizeAvailable && i == tableSizeAvailable))
				{
					x += tileSize + seperation;
					continue;
				}
